...
bool profileHandleToByteArray(EmoEngineEventHandle eProfile,
	...
	if (ok && *profileSize > 0) {
		...
		*profileBuffer = new unsigned char[*profileSize];
        ok = (EE_GetUserProfileBytes(eProfile, *profileBuffer, *profileSize) ==
                                                                        EDK_OK);
	...
	if (input.length()) {
		...
		// Print available commands
            ...
            os << "set_profile [profile name] [userID] \t set an existing profile"
                  " to a certain user ID" << std::endl;
            os << "get_profile [profile name] [userID] \t get the binary profile"
                  " of a certain user ID" << std::endl;
			...
			if (commands.size() == 3) {
                ...
                if (convert(commands.at(2), userID) &&
                    ...
                    if (profileManager.extractProfile(profileName, profileBuffer,
                        ...
                        wrongArgument = EE_SetUserProfile(userID, profileBuffer,
                                                          profileSize) != EDK_OK;
			...
			if (commands.size() == 3) {
                ...
                if (convert(commands.at(2), userID) &&
					...
					if (EE_GetUserProfile(userID, eProfile) == EDK_OK) {
                        ...
                        if (profileHandleToByteArray(eProfile, &profileBuffer,
                                                     &profileSize)) {
