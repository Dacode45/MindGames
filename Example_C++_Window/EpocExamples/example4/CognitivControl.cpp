	...
	if (input.length()) {
		...
		// Print available commands
            ...
            const std::string activeActionList = "\"push\",\"pull\",\"lift\",\""
                    "drop\",\"left\",\"right\",\"rotate_left\",\"rotate_right\",\""
                    "rotate_clockwise\",\"rotate_counter_clockwise\",\""
                    "rotate_forwards\",\"rotate_reverse\",\"disappear\"";
            ...
            os << "training_action [userID] [action] \t set Cognitiv training action"
               << std::endl;
            os << "\t\t\t\t\t [action: \"neutral\"," << activeActionList  << "]"
               << std::endl;
			
            os << "training_start [userID] \t\t start Cognitiv training"
               << std::endl;
            os << "training_accept [userID] \t\t accept previous Cognitiv training"
               << std::endl;
			os << "training_reject [userID] \t\t reject previous Cognitiv training";
            os << "training_erase [userID] \t\t erase Cognitiv training data"
		...
		// Set Cognitiv Active actions
			...
			if (commands.size() <= 6) { //@@ set_actions userID a1 a2 a3 a4
				...
				if (convert(commands.at(1), userID)) {
					...
					if (actionCheckOK) {
                        os << "Setting Cognitiv active actions for user "
                           << userID << "...";
						
                        wrongArgument = (EE_CognitivSetActiveActions(
		...
		// Change Cognitiv training action
			...
			if (commands.size() == 3) {
				...
				if (convert(commands.at(1), userID)) {
					...
					if (stringToAction(actionStr, &actionType)) {
                        os << "Setting Cognitiv training action for user "
                           << userID;
						os << " to " << actionStr << "...";
						
                        wrongArgument = (EE_CognitivSetTrainingAction(
		...
		// Start Cognitiv training
			...
			if (commands.size() == 2) {
				...
				if (convert(commands.at(1), userID)) {
                    ...
                    wrongArgument = (EE_CognitivSetTrainingControl(
                                         userID, COG_START) != EDK_OK);
				}
			}
		}
		...
		// Erase Cognitiv training data
			...
			if (commands.size() == 2) {
				...
				if (convert(commands.at(1), userID)) {
                    EE_CognitivAction_t actionType;
                    if (EE_CognitivGetTrainingAction(
                                userID, &actionType) == EDK_OK) {
                        os << "Erasing Cognitiv training for action \""
                           << actionToString(actionType) << "\" for user "
                           << userID << "...";
                        
                        wrongArgument = (EE_CognitivSetTrainingControl(
                                             userID, COG_ERASE) != EDK_OK);
                    }
				}
			}
		}
