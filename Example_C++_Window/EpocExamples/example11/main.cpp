...
** Example 11 - MultichanelEEGLogger
** This example is similar with the exaple 5( EEGLogger ) , except using the
** EE_DataGetMultiChannels function instead for EE_DataGet().
	...
	try {
		...
		if (argc != 2) {
            throw std::runtime_error("Please supply the log file name.\n"
                                     "Usage: MultichannelEEGLogger [log_file_name].csv");
		}
        ...
        std::cout << "Start receiving EEG Data! "
		...
		while (!_kbhit()) {
			...
			if (readytocollect) {						
                ...
                if (nSamplesTaken != 0) {
                    ...
                    EE_DataGetMultiChannels(hData, targetChannelList,
